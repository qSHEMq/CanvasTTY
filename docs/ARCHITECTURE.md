...
- `src/main/services/agent-runtime/` is a separate always-on lifecycle boundary and is not controlled by the Browser access switch. Every agent PTY receives a distinct capability for a protected user-local socket/pipe. Provider command hooks and the OpenCode event plugin may report only the fixed status enum, bounded event name, and optional opaque turn/prompt ID; prompt text, responses, tool input, and arbitrary telemetry are rejected by the exact gateway schema. Capabilities and temporary files are revoked when the PTY exits.
